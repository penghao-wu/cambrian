...
class CambrianMetaModel:
	...
	def initialize_vision_modules(self, model_args, fsdp=None):
		...
		if pretrain_mm_mlp_adapter is not None:
			...
			if compress_v:
				for layer_idx in range(compress_v_start_layer, self.config.num_hidden_layers):
					incompatible_keys = self.layers[layer_idx].vision_sampler_layers.load_state_dict(get_w(mm_projector_weights, 'layers.{}.vision_sampler_layers'.format(layer_idx)),strict=True)
