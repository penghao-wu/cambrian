	...
	def initialize_vision_modules(self, model_args, fsdp=None):
		...
		if pretrain_mm_mlp_adapter is not None:
			...
			def get_w(weights, keyword):
				return {k.split(keyword + '.')[1]: v.to(torch.float32) for k, v in weights.items() if keyword+'.' in k}
...
class CambrianMetaForCausalLM(ABC):
	...
	def prepare_inputs_labels_for_multimodal(
		...
		newline_embeds = self.get_model().image_newline.view(1, 1, -1).repeat(bs, max_num_image_crops*int(per_crop_token_len**0.5), 1)
