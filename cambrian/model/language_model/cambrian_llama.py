...
class CambrianLlamaModel(CambrianMetaModel, LlamaModel):
	...
	def cal_aux_loss(self, layer_id, vision_full, vision_concise, vision_full_side_len, vision_concise_side_len, valid_mask):
		...
		mse_squared_error = mse_squared_error/(num_valid+0.0001)
	...
	def forward(
		...
		# skip_layers = [_ for _ in range(0, 32)]
		skip_layers = [_ for _ in range(14, 40)]
		...
		for i, decoder_layer in enumerate(self.layers):
			...
			if i in skip_layers:
				...
				# 		hidden_states_text,
				# 		attention_mask_c2f,
				# 		position_ids_sys, position_ids_vision_concise, position_ids_vision_full, position_ids_vision_text,
