	...
	def forward(
		...
		for layer_i, decoder_layer in enumerate(self.layers):
			...
			if not compress_v or layer_i < compress_v_start_layer:
				...
				hidden_states_image_full = layer_outputs[-1]
...
class CambrianQwenForCausalLM(Qwen2ForCausalLM, CambrianMetaForCausalLM):
	...
	def forward(
		...
		aux_loss_total = outputs.aux_loss * 5
...
def decoder_forward(
	...
	**kwargs,):
		if fast_vision:
			...
			hidden_states_image_full_residual = hidden_states[:, len_image_compress:len_image_compress+len_image_full]
			hidden_states = self.input_layernorm(hidden_states)
			kv_states = hidden_states
