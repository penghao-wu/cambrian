...
# 		# skip_layers = [_ for _ in range(0, 32)]
# 		skip_layers = [_ for _ in range(23, 24)]
# 		skip_layers = []
...
class CambrianQwenForCausalLM(Qwen2ForCausalLM, CambrianMetaForCausalLM):
	...
	def forward(
		...
		if IS_XLA_AVAILABLE:
			...
			position_ids=torch.cat([position_ids_sys, position_ids_vision_full, position_ids_vision_text], dim=1),
			past_key_values=past_key_values,
			inputs_embeds=inputs_embeds,
			use_cache=use_cache,
			output_attentions=output_attentions,
...
from transformers.models.qwen2.modeling_qwen2 import Qwen2SdpaAttention, apply_rotary_pos_emb, repeat_kv
