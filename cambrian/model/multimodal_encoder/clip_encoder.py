...
class ClipVisionTower(BaseVisionTower):
    ...
    def load_model(self, device_map=None):
        ...
        self.image_processor.crop_size['height'] = 336*2
        self.image_processor.crop_size['width'] = 336*2
        self.image_processor.size['shortest_edge'] = 336*2
    ...
    def _forward(self, images):
        ...
        with torch.set_grad_enabled(self.unfreeze_mm_vision_tower):
            ...
            image_forward_outs = self.vision_tower(images.to(device=self.device, dtype=self.dtype), output_hidden_states=True)
            image_features = self.feature_select(image_forward_outs).to(images.dtype)
            _, num_tokens, dim = image_features.shape
