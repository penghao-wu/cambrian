...
def prepare_image_information(per_crop_token_len, compress_reduce_factor, image_size=None, is_dummy=False, dummy_num=1, max_length=2048):
	...
	if is_dummy:
		...
		position_ids_newline_full = torch.full((dummy_num,), max_length-1, dtype=torch.long)
		...
		if left_offset > 0:
			attention_mask_image_full_withnewline[:, :left_offset] = 0
		if right_offset > 0:
			attention_mask_image_full_withnewline[:, -right_offset-1:-1] = 0
		if top_offset > 0:
			attention_mask_image_full_withnewline[:top_offset, :]=0
		if bottom_offset > 0:
			attention_mask_image_full_withnewline[-bottom_offset:, :] = 0
		attention_mask_image_full_withnewline = attention_mask_image_full_withnewline.flatten()
		position_ids_image_full_withnewline = (attention_mask_image_full_withnewline.cumsum(0)-1).to(torch.long)
		attention_mask_image_full_withnewline = attention_mask_image_full_withnewline.view(height, (width+1))
		position_ids_image_full_withnewline = position_ids_image_full_withnewline.view(height, (width+1))

		attention_mask_image_full = attention_mask_image_full_withnewline[:, :-1].flatten()
		attention_mask_newline_full = attention_mask_image_full_withnewline[:, -1:].flatten()
		position_ids_image_full = position_ids_image_full_withnewline[:, :-1].flatten()
		position_ids_newline_full = position_ids_image_full_withnewline[:, -1:].flatten()

		left_offset, right_offset, top_offset, bottom_offset = get_padding_offset((height_compress, width_compress), image_size)
		attention_mask_image_compress_withnewline = torch.ones((height, (width+1)), dtype=torch.bool)
		if left_offset > 0:
			attention_mask_image_compress_withnewline[:, :left_offset] = 0
		if right_offset > 0:
			attention_mask_image_compress_withnewline[:, -right_offset-1:-1] = 0
		if top_offset > 0:
			attention_mask_image_compress_withnewline[:top_offset, :]=0
		if bottom_offset > 0:
			attention_mask_image_compress_withnewline[-bottom_offset:, :] = 0
		attention_mask_image_compress_withnewline = attention_mask_image_compress_withnewline.flatten()
		position_ids_image_compress_withnewline = (attention_mask_image_compress_withnewline.cumsum(0)-1).to(torch.long)
		attention_mask_image_compress_withnewline = attention_mask_image_compress_withnewline.view(height_compress, (width_compress+1))
		position_ids_image_compress_withnewline = position_ids_image_compress_withnewline.view(height_compress, (width_compress+1))

		attention_mask_image_compress = attention_mask_image_compress_withnewline[:, :-1].flatten()
		attention_mask_newline_compress = attention_mask_image_compress_withnewline[:, -1:].flatten()
		position_ids_image_compress = position_ids_image_compress_withnewline[:, :-1].flatten()
		position_ids_newline_compress = position_ids_image_compress_withnewline[:, -1:].flatten()
...
def prepare_multimodal_data(input_ids, labels, attention_mask, max_num_image_crops, per_crop_token_len, compress_reduce_factor, max_length=2048, pad_token_id=0):
	...
	for batch_idx, cur_input_ids in enumerate(input_ids):
		...
		if num_image_crops == 0:
			...
			position_ids_image_full.append(torch.full((max_num_image_crops*per_crop_token_len,), max_length-1, dtype=torch.long))
			...
			position_ids_newline_full.append(torch.full((max_num_image_crops,), max_length-1, dtype=torch.long))
		...
		if num_image_crops < max_num_image_crops:
			...
			cur_image_info_dummy = prepare_image_information(per_crop_token_len, compress_reduce_factor, is_dummy=True, dummy_num=num_image_crops_padded, max_length=max_length)
